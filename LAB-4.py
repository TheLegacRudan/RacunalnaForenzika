import webbrowser
from pypdf import PdfReader, PdfWriter
import os, sys, optparse
from exif import Image

def pdfMetaData(file_path):
    pdf_doc = PdfReader(open(path, "rb"))
    if pdf_doc.is_encrypted:
        pdf_doc.decrypt("")
    pdfWriter = PdfWriter()
    for pageNum in pdf_doc.pages:
        pdfWriter.add_page(pageNum)
    with open('decrypted_output.pdf', 'wb') as f:
        pdfWriter.write(f)

def figMetaData(file_path):
    img_doc = Image(open(file_path, "rb"))
    if not img_doc.has_exif:
        sys.exit(f"No EXIF data.")
    else:
        print(f"Image EXIF {img_doc.exif_version} data.")
    print(f"{dir(img_doc)}\n")
    decimalLong = convertGPScoordinate(img_doc.gps_longitude, img_doc.gps_longitude_ref)
    decimalLat = convertGPScoordinate(img_doc.gps_latitude, img_doc.gps_latitude_ref)
    print(f"Lat: {decimalLat} Long: {decimalLong}")
    url = f"http://www.google.com/maps/place/{decimalLat},{decimalLong}"
    webbrowser.open_new_tab(url)

def convertGPScoordinate(coordinate, coordinate_ref):
    decimal_degrees = coordinate[0] + \
                      coordinate[1] / 60 + \
                      coordinate[2] / 3600
    
    if coordinate_ref == "S" or coordinate_ref == "W":
        decimal_degrees = -decimal_degrees
    return decimal_degrees

if __name__ == "__main__":
    parser = optparse.OptionParser("Usage: python <script_name> -f <file>")
    parser.add_option("-f", dest="file", type="string", help="")
    (options, args) = parser.parse_args()
    path = options.file
    if not path:
        sys.exit(parser.usage)
    if any(path.endswith(ext) for ext in (".jpg", ".bmp", ".jpeg",)):
        figMetaData(path)
    elif path.endswith(".pdf"):
        pdfMetaData(path)
    else:
        print("error")